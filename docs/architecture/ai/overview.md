...
- 현재 구현된 DB 예산 서비스는 일·월 window를 고정 순서로 잠그고 같은 transaction에서 예약하며, 미전송 만료 건은 해제하고 전송 가능성이 있는 만료 건은 최대 비용으로 정산한다. Responses adapter는 후보 ID를 서버에서 다시 검사하고 alias 대신 snapshot 설정을 요구한다. 일반·browser Worker의 처리 콜백은 이제 추출 뒤 분류 결과를 받아야 `READY`로 끝나며, 예산 소진·무효 응답은 `PARTIAL`로 끝난다. 단, 현재는 테스트용 구성 주입만 검증했다. 실제 taxonomy 후보 공급, Worker runtime 조립, 예약 reconciler 주기 실행, 80% 알림, 승인된 model snapshot 검증, 분류 결과와 최종 상태의 단일 transaction 반영이 남아 있어 아직 출시 가능한 보호로 간주하지 않는다.
