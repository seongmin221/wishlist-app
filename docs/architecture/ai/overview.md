...
- 현재 구현된 DB 예산 서비스는 일·월 window를 고정 순서로 잠그고 같은 transaction에서 예약하며, 미전송 만료 건은 해제하고 전송 가능성이 있는 만료 건은 최대 비용으로 정산한다. Responses adapter는 후보 ID를 서버에서 다시 검사하고 alias 대신 snapshot 설정을 요구한다. 이 단계는 아직 Worker의 최종 `READY` 경로와 연결되지 않았으며, 실제 snapshot ID 검증·분류 결과 저장·운영 알림과 함께 연결해야 출시 가능한 보호가 된다.
