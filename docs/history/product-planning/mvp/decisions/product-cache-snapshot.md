...
- MVP에서는 canonical URL이 같은 `READY` 캐시만 추출 완료 시점부터 7일 동안 재사용한다. `PARTIAL`과 모든 실패 캐시는 재사용하지 않고 새 저장 요청의 대상만 분석한다.
...
공용 `Product`를 사용하는 목적은 이미 추출한 상품 정보를 재사용해 새 항목을 빠르게 만드는 것이다. `READY` 결과만 7일로 제한해 재사용하면 초기 MVP에서도 중복 fetch·파싱·AI 분류 비용과 대기 시간을 줄이면서, 오래된 상품 정보를 무기한 사용하는 문제를 피할 수 있다. 가격은 마지막 확인 시점을 함께 표시한다. 이를 `WishlistItem`의 실시간 원본으로 사용하면 다른 저장 요청이나 캐시 보완 때문에 사용자가 보고 있던 상품 정보가 예고 없이 달라질 수 있다.
...
- 캐시 만료 판정과 `READY` 캐시 복사 처리는 신규 저장 pipeline에만 적용한다.
