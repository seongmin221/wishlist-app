# Documentation Index

AI 탐색의 최상위 진입점이다. 원문 검색을 대체하지 않고, 영역과 문서 유형을 빠르게 좁힌다.

| 영역 | 목적 | Index |
| --- | --- | --- |
| Product | 기능 범위와 제품 규칙, 출시 기준 taxonomy | [product/INDEX.md](product/INDEX.md), [product-spec.md](product-spec.md), [product/references/product-taxonomy.md](product/references/product-taxonomy.md) |
| Architecture | client, server, AI, repository 구조 | [architecture/INDEX.md](architecture/INDEX.md) |
| Learning | 프로젝트 맥락의 기술 학습 | [learning/INDEX.md](learning/INDEX.md) |
| History | 결정과 구현 사유 | [history/INDEX.md](history/INDEX.md) |

작성 규칙과 문서 상태 표기는 [README.md](README.md)를 참고한다.
