...
> 상태: **제안** · 근거: 이전 통합 스펙의 traceability와 연결된 MVP 결정 기록
...
일반 카테고리 목록에서 상품을 사용하기 위한 최소 정보는 제품명과 카테고리다. 대표 이미지, 가격, 통화, 판매처, 브랜드와 목적은 선택 정보다. `PARTIAL` 또는 `FAILED` 추출 결과라도 사용자가 제품명과 카테고리를 직접 보완하면 `정보 보완 필요` 영역에서 벗어날 수 있다. 상세 동선은 [상품 확인과 편집](../inspect-and-edit-a-product.md)이 소유하고, 결정 근거는 [상품 정보 직접 보완의 완료 기준](../../history/product-planning/mvp/decisions/manual-completion.md)에 보존한다.
