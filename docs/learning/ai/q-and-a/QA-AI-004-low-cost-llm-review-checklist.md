...
- category와 purpose를 한 요청으로 받을지 두 요청으로 나눌지 명확히 정한다. 저비용 MVP에서는 두 결과를 하나의 구조화 응답으로 받는 편이 입력 중복, 부분 실패와 재시도를 줄인다.
- `Product` metadata 캐시와 AI 결과 캐시를 혼동하지 않는다. 현재 `Product` 캐시는 추출 metadata만 재사용하며, 캐시 적중만으로 사용자별 category·purpose 판단을 생략하지 않는다. 별도 AI 결과 캐시를 도입하지 않는 것도 명시적인 선택이다.
...
- 입력·출력 토큰, description 길이, purpose 후보 수와 문맥 수를 제한하고, 일·월 예산은 알림뿐 아니라 초과 시 AI 호출을 멈추는 정책까지 정한다.
